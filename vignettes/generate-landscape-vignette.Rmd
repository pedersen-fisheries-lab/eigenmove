...
The next step is to use eigenvalues and eigenvectors of the movement matrix to calculate kinetic distances between points using the function `calculate_kinetic_distances()`. This function uses the first $d+1$ eigenvalues and eigenvectors to use for the kinetic distance calculation. A higher `d` will give a more accurate result (**up to a certain point**), but will take longer to compute. The argument `T` is a time parameter where higher `T` values will emphasize slower modes of movement, corresponding to larger scale landscape structures.
