...
In this vignette you will see a complete example of eigenmove's capabilities. We will show you how to generate a toy landscape using Gaussian Processes (`create_GP_landscape()`) and how to compute random walks on that landscape according to a simple movement model (`em_loadlandscape()`, `calc_step()`, `rescale_landscape()`). This will result in a *movement matrix* which can then be processed through the core `eigenmove` functionality of estimating patches on a landscape. If you already have a movement matrix and landscape raster and just want to skip to estimating patches, see `vignette("eigenmove-vignette")`.

### Creating a toy landscape

Use `create_GP_landscape()` to simulate a toy landscape via a Gaussian process for use with a simple random walk movement model. In this context a Gaussian Process (GP) assumes that an "intensity" value (or pixel darkness) at each point on the landscape follows a normal distribution, and values for points close to one another are correlated, so they have similar values. For more information on GPs see this article: <https://distill.pub/2019/visual-exploration-gaussian-processes/>.
...
This generates a dataframe with 3 columns: an x-coordinate a y-coordinate and a GP-generated "intensity" value for each point of a landscape. To convert "intensity" into simple low, mid and high "habitat qualities" we can run `rescale_landscape()` to generate a new column (`type`) based on the generated values.
...
For a $32 \times 32$ image this results in a $n \times n = 32^2 \times 32^2 = 1024 \times 1024$ movement matrix. Each pixel of the image has a column in the matrix, and each entry of that column describes the movement probability of moving from that pixel to the pixel of the corresponding row. Movement matrices of this type can be generated a number of ways. For this reason we included a separate vignette to explain the eigenmove core functionality for those who bring their own movement matrix: `vignette("eigenmove-vignette")`. Let's move on here with our generated landscape.

The next step is to use eigenvalues and eigenvectors of the movement matrix to calculate kinetic distances between points using the function `calculate_kinetic_distances()`. This function computes the first $d+1$ eigenvalues, left eigenvectors, and right eigenvectors to use for the kinetic distance calculation. Eigenvalues are stored in a $d+1$ vector. Right and left eigenvectors are stored in separate $n \times d + 1$ matrices. A higher `d` will give a more accurate result (**up to a certain point**), but will take longer to compute. The argument `T` is a time parameter (\$\\tau\$ from the companion theory paper) where higher `T` values will emphasize slower modes of movement, corresponding to larger scale landscape structures.
...
This results in another $n \times n$ (1024 x 1024) matrix with columns and rows representing respective pixel origins and destinations, like the movement matrix. The entries in this matrix represent lower-dimensional interaction rates between pixels based on lifetime encounter probabilities of individuals starting on those pixels.

Finally, we can cluster this matrix to estimate patches using a clustering algorithm. We have included two common algorithms in the function `calculate_clusters()` via the `cluster_type` argument: agglomerative hierarchical clustering (`hclust`), and "Density-based Spatial Clustering of Applications with Noise" (`DBSCAN`).
...
Here is an `hclust` result where we would like the algorithm to specify *2 patches*. 
```{r}
small_landscape_hclust <- calculate_clusters(
  ...
  n_clust = 2) # Split the landscape into 2 patches

small_landscape_hclust_plot <- ggplot(
  data = dplyr::filter(small_landscape_hclust, in_patch), aes(x = x, y = y)) +  
...
Here is a different `hclust` result where we would like the algorithm to specify *5 patches*. 
```{r}
small_landscape_hclust <- calculate_clusters(
  cluster_type = "hclust",
  landscape = small_landscape, # original landscape dataframe
...
2.  DBSCAN requires two parameters: `eps` (the maximum distance between two samples for one to be considered as in the neighborhood of the other) and `minPts` (the required number of samples in a neighborhood for a point to be considered as a core point).

Here is a `DBSCAN` result tuning `eps` and `minPts` to ?? (SEE ISSUE #9 IN GITHUB)
```{r}
...
small_landscape_dbscan_plot <- ggplot(
  data = dplyr::filter(small_landscape_dbscan, in_patch), aes(x = x, y = y)) +  
  geom_raster(aes(fill = factor(clusters))) +
  theme_minimal()

print(small_landscape_dbscan_plot)
```

Here is a different `DBSCAN` result tuning `eps` and `minPts` to ?? (SEE ISSUE #9 IN GITHUB)
```{r}
small_landscape_dbscan <- calculate_clusters(
  cluster_type = "DBSCAN",
  landscape = small_landscape, # original landscape dataframe
...
Other clustering algorithms can be used. The function `calculate_kinetic_distances()` returns a list which includes a *distance matrix* object (`small_landscape_em$dists` here)  - this is the type of object required by most clustering algorithms. 
